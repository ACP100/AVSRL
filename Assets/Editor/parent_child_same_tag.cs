...
public class ChangeChildTags : MonoBehaviour {
    ...
    public static void ChangeChildrenTags()
    {
        ...
        {
            if(EditorUtility.DisplayDialog("Change child tags to parent tag", 
               "Do you really want to change every child tag to " + parentTag + "?", 
               "Change tags", "Cancel"))
            {
                ...
                {
                    counter++;
                    EditorUtility.DisplayProgressBar("Changing tags", 
                        "Changing all child object tags to " + parentTag + 
